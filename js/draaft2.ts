  ...
  fetch("http://localhost:8000/user", {
    ...
    .then(async (json) => {
      document.getElementById(
        "menu-welcome-text"
      ).innerText = `welcome, ${json.username}`;
    })
    .catch((error) => {
...
async function testAuthToken(auth: string) {
  const interval = new UpdatingText(
    "login-response-text",
    "contacting drAAft server..",
    25,
    false
  );
  fetch("http://localhost:8000/authenticated", {
    ...
    .then(async (text) => {
      ...
      if (text != "true") {
        new UpdatingText(
          "login-response-text",
          "incorrect login token, try copying again",
          20,
          true
        );
      } else {
    ...
    .catch((error) => {
      ...
      new UpdatingText(
        "login-response-text",
        "error encountered with login, try copying again",
        20,
        true
      );
