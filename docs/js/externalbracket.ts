...
import {displayOnlyPage, fullPageNotification, play_audio, removeAllPages, ROOM_CONFIG, STEVE, UUID, set_draft_info, IS_PLAYER} from "./draaft2/util.js";
...
function toHelmet(name: string) {
    return `https://mineskin.eu/helm/${checkedName(name)}/100`;
}

function setupBracket(bk: any) {
    window.bracketsViewer.setParticipantImages(bk.participants.map(participant => ({
            participantId: participant.id,
            imageUrl: toHelmet(participant.name),
        })));
    window.bracketsViewer.render(bk, {
        selector: '#' + 'bracket-view-div',
...
export let BRACKET = undefined;
export function fetchData(): Promise<void> {
    return requiredJsonGETRequest("bracket/external/current", (json) => {
            console.log("Successfully downloaded bracket.");
            if (LOCAL_TESTING) {
                console.log(json);
            }
            BRACKET = json;
...
function main() {
    ...
    (<HTMLDialogElement>document.getElementById("loading-credits")).show();
    fetchData().then(() => {
        console.log("Successfully loaded the bracket. Fading out credits.");
        setTimeout(() => {
            (<HTMLDialogElement>document.getElementById("loading-credits")).classList.add("fade");

            const id = "bracket-page";
            document.getElementById(id).style.display = "flex";
            document.getElementById(id).classList.add("visible", "fade");

            setTimeout(() => {(<HTMLDialogElement>document.getElementById("loading-credits")).close();}, 700);
        }, 1000);
    });
}
